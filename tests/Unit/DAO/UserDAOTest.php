...
class UserDAOTest extends TestCase
{
    ...
    public function it_should_get_students_by_year_and_section()
    {
        ...
        $this->pdoMock
            ->expects($this->once())
            ->method('prepare')
            ->with($this->stringContains("role = 'student'"))
            ->willReturn($this->pdoStatementMock);
        ...
        $result = $userDAO->getStudentsByYearAndSection($yearLevel, $section);
        
        $this->assertCount(2, $result);
        $this->assertContainsOnlyInstancesOf(User::class, $result);
        $this->assertEquals('John Doe', $result[0]->getFullName());
        $this->assertEquals('Jane Smith', $result[1]->getFullName());
    }

    /** @test */
    public function it_should_get_all_students()
    {
        $expectedStudents = [
            [
                'user_id' => 1,
        ...
        $this->pdoMock
            ->expects($this->once())
            ->method('prepare')
            ->with($this->stringContains("WHERE role = 'student'"))
            ->willReturn($this->pdoStatementMock);
    ...
    private function createUserDAOWithMockPDO()
    {
        ...
        $dbProperty->setValue($userDAO, $this->pdoMock);
        
        $tableProperty = $reflection->getProperty('table');
        $tableProperty->setAccessible(true);
        $tableProperty->setValue($userDAO, 'users');
