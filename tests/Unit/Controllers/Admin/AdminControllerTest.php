...
class AdminControllerTest extends TestCase
{
    ...
    /** @var \App\Services\Auth\AuthService&\PHPUnit\Framework\MockObject\MockObject */
    private $authServiceMock;
    /** @var \App\Services\User\UserService&\PHPUnit\Framework\MockObject\MockObject */
    private $userServiceMock;
    /** @var \App\Services\Subject\SubjectService&\PHPUnit\Framework\MockObject\MockObject */
    private $subjectServiceMock;
    /** @var \App\Services\Assignment\AssignmentService&\PHPUnit\Framework\MockObject\MockObject */
    private $assignmentServiceMock;
    /** @var \App\Core\View&\PHPUnit\Framework\MockObject\MockObject */
    private $viewMock;
    ...
    protected function setUp(): void
    {
        ...
        $this->subjectServiceMock = $this->createMock(\App\Services\Subject\SubjectService::class);
        $this->assignmentServiceMock = $this->createMock(\App\Services\Assignment\AssignmentService::class);
    ...
    public function it_should_handle_logout_with_confirmation()
    {
        ...
        // Since logout() calls exit(), we expect it to terminate execution
        // We just verify the mock was called - the exit() is expected behavior
     ...
     * @test
     */
    public function it_should_trigger_modal_logout_flow()
    {
        // In modal-only flow, without confirm=true the controller should set a session flag
        // and redirect back to the dashboard, not render a separate view.
        /** @var \App\Core\View&\PHPUnit\Framework\MockObject\MockObject $viewMock */
        $viewMock = $this->createMock(View::class);
        $viewMock->expects($this->never())->method('display');
        ...
        // Since logout() calls exit(), we expect it to terminate execution
        // We just verify the session flag was set before exit()
        $this->expectOutputString('');
        ...
        // This assertion may not be reached due to exit(), but that's expected
        $this->assertArrayHasKey('show_logout_modal', $_SESSION);
        $this->assertTrue((bool)$_SESSION['show_logout_modal']);
    }
    ...
    /**
     * Helper to invoke private/protected methods for tests that call $this->invokePrivateMethod(...)
     *
    ...
    private function invokePrivateMethod(string $methodName, array $args = [])
    {
        ...
        $method->setAccessible(true);
        return $method->invokeArgs($this->adminController, $args);
    }
