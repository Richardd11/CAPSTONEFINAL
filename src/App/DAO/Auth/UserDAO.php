...
class UserDAO implements UserDAOInterface
{
    ...
    public function update($user_id, User $user): bool
    {
        try {
            ...
            if (!$result) {
                error_log("Failed to update user {$user_id}: " . implode(', ', $stmt->errorInfo()));
            }
     ...
     */
    public function getStudentsByYearAndSection($yearLevel, $section): array
    {
        try {
            ...
            error_log("Error getting students by year and section: " . $e->getMessage());
            return [];
        }
    }
    ...
    public function getAllStudents(): array
    {
        try {
            $stmt = $this->db->prepare("
                SELECT * FROM {$this->table} 
                WHERE role = 'student' 
                ORDER BY year_level ASC, section ASC, full_name ASC
            ");
            $stmt->execute();
